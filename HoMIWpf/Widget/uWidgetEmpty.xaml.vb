...
Public Class uWidgetEmpty
    ...
    Dim _PositionStatus As Integer = 0
...
#Region "Property"
    ...
    Public Property PositionStatus As Integer
        Get
        ...
        Set(ByVal value As Integer)
            ...
            If _Show = False Then Exit Property
            Select Case _PositionStatus
                Case 0
                    StkEmptyetDevice.Orientation = Orientation.Horizontal
                Case 1
                    StkEmptyetDevice.Orientation = Orientation.Vertical
            End Select
        End Set
    End Property
                            ...
                            Select Case _ElmtVisu.Operateur
                                Case cWidget.Operateur.Diff
                                    If retour <> _ElmtVisu.Value Then
                                        ...
                                        If String.IsNullOrEmpty(_ElmtVisu.Status) = False Then
                                            If InStr("#?", _ElmtVisu.Status) = 0 Then
                                                LblStatus.Content = _ElmtVisu.Status
                                            Else
                                                LblStatus.Content = ""
                                            End If
                                        End If
...
#End Region
    ...
    Private Sub Stk1_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Input.MouseButtonEventArgs) Handles Me.MouseDown
        Try
            ...
            Dim multwidth As Double = 1
            Dim multheight As Double = 1
            ...
            If _ModeEdition Then
                ...
                End Try
            Else
                multwidth = Me.Width / 140
                multheight = Me.Height / 100
                ...
                If IsEmpty = False Then
                    ...
                    End If
                    If StkPopup.Children.Count > 0 Then
                        '    AfficheMessageAndLog(FctLog.TypeLog.INFO, "Je suis la " & mult & " / " & Popup1.Height)
                        Popup1.Width = 140
                        Popup1.Height = 60
                        Popup1.Width = Popup1.Width * multwidth
                        Popup1.Height = Popup1.Height * multheight
                ...
                Else
                    If StkPopup.Children.Count > 0 Then
                        ' AfficheMessageAndLog(FctLog.TypeLog.INFO, "Je suis ici " & Popup1.Width.ToString)
                        If Popup1.IsOpen = False Then
                            ' AfficheMessageAndLog(FctLog.TypeLog.INFO, "Je suis ici " & mult & " \ " & Popup1.Width.ToString)
                            Popup1.Width = 140
                            Popup1.Width = Popup1.Width * multwidth
                            Popup1.IsOpen = True
                            Else
    ...
    Private Sub uWidgetEmpty_SizeChanged(ByVal sender As Object, ByVal e As System.Windows.SizeChangedEventArgs) Handles Me.SizeChanged
        Try
            ...
            If ShowStatus And (ShowPicture Or _type = TypeOfWidget.Moteur Or Type = TypeOfWidget.Prise) And LblStatus.ActualHeight > 0 And Me.ActualWidth > 0 Then
                If LblStatus.IsVisible Then
                    Select Case _PositionStatus
                        Case 0 : W = Me.ActualWidth - LblStatus.ActualWidth
                        Case 1 : W = Me.ActualWidth
                    End Select
                End If
                If W < 0 Then W = 0
        ...
        Try
            ...
            If _dev IsNot Nothing Then
                ...
                Else
                    ...
                    DisplayPictureFileName = DisplayPictureFileName.Replace(_MonRepertoire, "")
                    DisplayPictureFileName = myService.GetRepertoireOfServer & DisplayPictureFileName
