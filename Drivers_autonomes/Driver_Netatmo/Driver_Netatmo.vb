...
line1:
                ...
                WriteLog("DBG: Scan data responseby : " & responsebody)
    ...
    Private Sub GetData(ByVal objet As Object)
        ...
        Try
            ...
            WriteLog("DBG: GetData => Nombre de device = " & devlist.body.devices.Count)
            For Each _Dev In devlist.body.devices
                WriteLog("DBG: GetData => _Dev.module_name = " & _Dev.module_name & " / " & _Dev.type)
                If objet.adresse1 = _Dev.module_name Then
                    deviceIDalire = _Dev
                    Typealire = _Dev.type
                    Exit For
                End If
                WriteLog("DBG: GetData => Nombre de modules = " & _Dev.modules.Count)
                For Each _Mod In _Dev.modules
                    WriteLog("DBG: GetData => _Mod.module_name = " & _Mod.module_name & " / " & _Mod.type)
        ...
        Catch ex As Exception
            ...
            WriteLog("ERR: GetData=> Cette erreur peut survenir lors de la déconnexion de la base : " & ex.Message)
        End Try
    End Sub
