    ...
    Public Sub Read(ByVal Objet As Object) Implements HoMIDom.HoMIDom.IDriver.Read
        ...
        Try
            ...
            Select Case Objet.Type
                Case "LAMPE", "APPAREIL"   'relais
                    If _IPXVersion = "IPX800_V4" Then
                        valeur = GET_VALUE_V4(_Username, _Password, "R", Objet.Adresse1)
                    Else
                        valeur = GET_VALUE("OUT" & Objet.Adresse1)
                    End If
                    If valeur <> "" Then Objet.Value = (valeur = 1)
                Case "CONTACT"   'entree digit
                    If _IPXVersion = "IPX800_V4" Then
                        If InStr(Objet.Adresse1, "V") > 0 Then 'entree virtuelle
                            valeur = GET_VALUE_V4(_Username, _Password, "VI", Mid(Objet.Adresse1, 2, Len(Objet.Adresse1)))
                        Else
                            valeur = GET_VALUE_V4(_Username, _Password, "D", Objet.Adresse1)
                        End If
                    Else
                ...
                Case "COMPTEUR"
                    If String.IsNullOrEmpty(Objet.adresse2) Then
                        If _IPXVersion = "IPX800_V4" Then
                            valeur = GET_VALUE_V4(_Username, _Password, "C", Objet.Adresse1)
                        Else
                    ...
                    Else
                        ...
                        Dim numcpt As String = Mid("00", 1, 2 - Len(Objet.adresse1)) & Objet.adresse1
                        If _IPXVersion = "IPX800_V4" Then
                            Select Case True
                                Case InStr(Objet.adresse2, "inc=") > 0 : SEND_IPX800(_urlIPX & "api/xdevices.json?key=apikey&SetC" & numcpt & "=+" & Mid(Objet.adresse2, InStr(Objet.adresse2, "=") + 1, Len(Objet.adresse2)), _Username, _Password)
                                Case InStr(Objet.adresse2, "dec=") > 0 : SEND_IPX800(_urlIPX & "api/xdevices.json?key=apikey&SetC" & numcpt & "=-" & Mid(Objet.adresse2, InStr(Objet.adresse2, "=") + 1, Len(Objet.adresse2)), _Username, _Password)
                                Case (InStr(Objet.adresse2, "inc=") = 0) And (InStr(Objet.adresse2, "dec=") = 0) : SEND_IPX800(_urlIPX & "api/xdevices.json?key=apikey&SetC" & numcpt & "=" & Mid(Objet.adresse2, InStr(Objet.adresse2, "=") + 1, Len(Objet.adresse2)), _Username, _Password)
                            End Select
                            valeur = GET_VALUE_V4(_Username, _Password, "C", Objet.Adresse1)
                            If valeur <> "" Then Objet.Value = valeur
                        Else
                            ...
                            If (InStr(Objet.adresse2, "inc=") > 0) Or (InStr(Objet.adresse2, "dec=") > 0) Then
                                SEND_IPX800(_urlIPX & "protect/assignio/counter" & numpage & ".htm?num=" & Objet.adresse1 - 1 & "&" & Mid(Objet.adresse2, InStr(Objet.adresse2, "=") + 1, Len(Objet.adresse2)), _Username, _Password)
                            Else
                        ...
                        End If
                    End If
                Case "TEMPERATURE", "HUMIDITE", "ENERGIEINSTANTANEE", "UV"
                    If _IPXVersion = "IPX800_V4" Then  'entree ana
                        Select Case True
                            Case InStr(Objet.Adresse1, "V") > 0 'entree virtuelle
                                valeur = GET_VALUE_V4(_Username, _Password, "VA", Mid(Objet.Adresse1, 2, Len(Objet.Adresse1)))
                                WriteLog("DBG: Valeur recherchée : " & Objet.Type & ", key= VA -> " & Objet.Adresse1)
                            Case InStr(Objet.Adresse1, "THL") > 0 'entree sonde xth
                                WriteLog("DBG: Valeur recherchée : " & Objet.Type & ", key= THL -> " & Objet.Adresse1)
                                If Objet.Adresse1 = "THL" Then Objet.Adresse1 = "THL1"
                                Select Case True
                                    Case Objet.Type = "TEMPERATURE"
                                        valeur = GET_VALUE_V4(_Username, _Password, "THL", Mid(Objet.Adresse1, 1, 4) & "-TEMP")
                                    Case Objet.Type = "HUMIDITE"
                                        valeur = GET_VALUE_V4(_Username, _Password, "THL", Mid(Objet.Adresse1, 1, 4) & "-HUM")
                                    Case Objet.Type = "UV"
                                        valeur = GET_VALUE_V4(_Username, _Password, "THL", Mid(Objet.Adresse1, 1, 4) & "-LUM")
                                End Select
                            Case Else
                                valeur = GET_VALUE_V4(_Username, _Password, "A", Objet.Adresse1)
                                WriteLog("DBG: Valeur recherchée : " & Objet.Type & ", key= A -> " & Objet.Adresse1)
                        End Select
                    Else
                ...
                Select Case Objet.Type
                    Case "LAMPE", "APPAREIL"
                        Select Case Command
                            Case "ON"
                                If _IPXVersion = "IPX800_V4" Then
                                    If InStr(Objet.Adresse1, "V") > 0 Then 'sortie virtuelle
                                        SEND_IPX800(_urlIPX & "api/xdevices.json?key=apikey&SetVO=" & Mid(Objet.Adresse1, 2, Len(Objet.Adresse1)), _Username, _Password)
                                    Else
                                        SEND_IPX800(_urlIPX & "api/xdevices.json?key=apikey&SetR=" & Objet.adresse1, _Username, _Password)
                                    End If
                                Else
                            ...
                            Case "OFF"
                                If _IPXVersion = "IPX800_V4" Then
                                    If InStr(Objet.Adresse1, "V") > 0 Then 'sortie virtuelle
                                        SEND_IPX800(_urlIPX & "api/xdevices.json?key=apikey&ClearVO=" & Mid(Objet.Adresse1, 2, Len(Objet.Adresse1)), _Username, _Password)
                                    Else
                                        SEND_IPX800(_urlIPX & "api/xdevices.json?key=apikey&ClearR=" & Objet.adresse1, _Username, _Password)
                                    End If
                                Else
                                    If _IPXOldVersion Then
                    ...
                    Case "SWITCH"
                        Select Case Command
                            Case "ON"
                                If _IPXVersion = "IPX800_V4" Then
                                    If InStr(Objet.Adresse1, "V") > 0 Then 'sortie virtuelle
                                        SEND_IPX800(_urlIPX & "api/xdevices.json?key=apikey&ToggleVO=" & Mid(Objet.Adresse1, 2, Len(Objet.Adresse1)), _Username, _Password)
                                    Else
                                        SEND_IPX800(_urlIPX & "api/xdevices.json?key=apikey&ToggleR=" & Objet.adresse1, _Username, _Password)
                                    End If
                                Else
                                    SEND_IPX800(_urlIPX & "preset.htm?set" & Objet.adresse1 & "=1", _Username, _Password)
                                End If
...
#Region "Fonctions internes"
    ...
    Private Sub GET_VALUES(adrs As String, user As String, password As String)
        ...
        Try
            ...
            adrs = _urlIPX & "api/xdevices.json?key=apikey&Get=all"
            ...
            If InStr(responsebody, """G1") > 0 Then ' cas IPX800 V4
                Dim Gx As String = ""
                Gx = Mid(responsebody, InStr(responsebody, """G1"), Len(responsebody))
                Gx = Mid(Gx, 1, InStr(Gx, """T1") - 1)
                responsebody = responsebody.Replace(Gx, "")
                WriteLog("DBG: GetValue inputs : " & responsebody.ToString)
                WriteLog("DBG: GetValue Gx : " & Gx.ToString)
            End If
    ...
    Public Function GET_VALUE_V4(user As String, password As String, key As String, adrs1 As String) As String
        Try
            'attend que l'update soit terminé
            While _UpdateInProcess
            End While
            ...
            Dim adrs As String = ""
            If key = "THL" Then
                adrs = _urlIPX & "api/xdevices.json?key=apikey&Get=XTHL"
            Else
                adrs = _urlIPX & "api/xdevices.json?key=apikey&Get=" + key
            End If
            WriteLog("DBG: " & "GET_VALUE_V4 Url: " & adrs)
            ...
            '  WriteLog("DBG: Get_Value_V4  : " & responsebody.ToString)

            Dim jsonObj = Newtonsoft.Json.JsonConvert.DeserializeObject(Of Dictionary(Of String, String))(responsebody)

            Dim valeur = ""
            For numberkey = 0 To jsonObj.Count - 1
                If jsonObj.Keys(numberkey).ToString = adrs1 Then
                    valeur = jsonObj.Item(jsonObj.Keys(numberkey)).ToString
                    Exit For
                End If
            Next
            Return valeur
        Catch ex As Exception
            WriteLog("ERR: " & "Get_Value_V4, Key " & key)
            WriteLog("ERR: " & ex.Message)
            Return ""
        End Try
    End Function
